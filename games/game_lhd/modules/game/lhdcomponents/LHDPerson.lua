...
function BrPerson:ctor()
    ...
    BrPerson.super.ctor(self, {id=PopupManager.POPUPWINDOW[self.TAG_ID], child =self.root })
    self:init()
    self:initTouch()
...
function BrPerson:update()
    ...
    local count = Cache.lhdinfo.others_count
...
function BrPerson:refreshList()
    local allInfo = clone(Cache.lhdinfo.others)
...
function BrPerson:updateItem(item, info)
    ...
    extparas.sq = true
    extparas.add = true
    local goldTxt = Util:getFormatString(gold)
    ...
    addButtonEvent(item,function (sender)
        qf.event:dispatchEvent(LHD_ET.GAME_SHOW_USER_INFO,{uin = info.uin, defaultImg = defaultImg,showGoldTxt = goldTxt})
    end) 
end
