local BrnnChips = class("BrnnChips") 

BrnnChips.TAG = "BrnnChips"
local ChipCls = require("src.common.Chip")
BrnnChips.defaultColor = cc.c3b(255,255,255)--cc.c3b(251,205,0)--cc.c3b(0,255,96)
BrnnChips.beautyColor = cc.c3b(255,0,0)

--筹码所对应的值
local valueT = {1,10,50,100,500,1000,5000}

function BrnnChips:init()
    ...
    local cacheNum = 100
    local colorArr = BrnnConstants.ColorIndex
    self.colorChipPools = {}
...
function BrnnChips:getChip(value)
...
function BrnnChips:putChip(chip)
...
function BrnnChips:create(value)
    local chip
    local scale = BrnnConstants.chipInPoolScale
    local fontScale = Cache.packetInfo:isRealGold() and BrnnConstants.chipFntScale_RealGold or BrnnConstants.chipFntScale_Gold
...
function BrnnChips:createT(value)
    local chips = {}
    --从大到小删除筹码
...
--保持接口不变 扩充新的函数
function BrnnChips:fly(delay,chip,from,to,cb)
    self:flyWithSpeed(delay,chip,from,to,0.0005,cb)
end

function BrnnChips:flyWithSpeed(delay,chip,from,to,speed,cb)
    self:_fly(delay, chip, from, to, cb, speed, false)
end

function BrnnChips:flyWithFixedTime(delay,chip,from,to,time,cb)
    self:_fly(delay, chip, from, to, cb, time, true)
end

--飞筹码的私有函数
function BrnnChips:_fly(delay,chip,from,to,cb,speed,isfixed)
    --当isfixed 为true时
...
function BrnnChips:getTempPosition(from,to,value)
...
function BrnnChips:getIndex(value)
...
function BrnnChips:updatePoolCacheChips()
    --默认从小到大
    local chipList = Cache.BrniuniuDesk.addChipList
    local colorArr = BrnnConstants.ColorIndex
...
function BrnnChips:dealWithChipsTask( ... )
    
end

BrnnChipManager = BrnnChips