--无座玩家页面
local BrnnPerson = class("BrnnPerson",CommonWidget.PopupWindow)


function BrnnPerson:ctor()
...
function BrnnPerson:show()
    self.super.show(self)
end

function BrnnPerson:initTouch()
...
function BrnnPerson:update()
    if self.root == nil then return end
    local count = Cache.BrniuniuInfo.others_count
...
function BrnnPerson:addItem(idx)
...
function BrnnPerson:refreshList()
    local allInfo = clone(Cache.BrniuniuInfo.others)
...
function BrnnPerson:updateHead(node,portrait,sex, extparas)
...
function BrnnPerson:updateItem(item, info)
    item:setVisible(true)
...
function BrnnPerson:cacheYList()
    self._cacheYList = {}
...
function BrnnPerson:showRange(iY)
    ...
    if self.yCacheBeginList then
        ...
        -- local yBegin = self._cacheYList[iBegin]
        -- local idx = table.indexof(self.yCacheBeginList, yBegin)
        -- if idx and idx + (iEnd - iBegin) <= #self.yCacheBeginList  then 
        --     return
        -- end
    end
...
function BrnnPerson:delayRun(time,cb)
...
function BrnnPerson:stopDelayRun()
