--[[
    百人场牌桌缓存
--]]
local BrniuniuDesk = class("BrniuniuDesk")


BrniuniuDesk.br_info = {}
BrniuniuDesk.br_user = {}
BrniuniuDesk.br_all_chips = {} --百人场下注的总数
BrniuniuDesk.br_my_chips = {} --自己下的注

function BrniuniuDesk:ctor ()
    self.name = BRC_MATCHE_TYPE
    self:clearCache()
end

function BrniuniuDesk:clearCache()
    -- self.roomid = 0
    self.deskid = 0
    self.br_info = {}
    self.br_user = {}
...
function BrniuniuDesk:getRoomConfig()
    --白人牛牛3倍场和10倍场 roomid，暂时是写死的
    return {40203, 40210}
end

function BrniuniuDesk:getRoomType( ... )
    if self.roomid == self:getRoomConfig()[2] then
        return 15
    end
    return 14 
end

function BrniuniuDesk:getLeastChipsPloolNum( ... )
    return Cache.BrniuniuDesk.min_bet_carry
end

--[[--
EvtDeskUserEnter 更新cache
]]
function BrniuniuDesk:updateCacheByBrInput( m )
    loga("百人牛牛进桌"  .. pb.tostring(m))
    self.roomid = m.roomid
    self.br_user = {}
    self.br_delar = {}
    ...
    self:updateBrnnUser(m, true)
    self:updateBrnnChipsPool(m.br)
    self:updateBrAddChipsInfo(m.br)
    self.jackpot_amount_changed = true
    self.min_bet_carry = m.min_bet_carry
end

function BrniuniuDesk:updateBrAddChipsInfo(model)
...
function BrniuniuDesk:updateBrInfo(model)
    local propsTable = {"stage","time_remain"}
    self:_updateProps(propsTable,model,self.br_info)
end

function BrniuniuDesk:_updateProps( propsTable , src,dest )
...
--百人场发牌
function BrniuniuDesk:updateBrnnShareCards(m)
    self.br_sharecards = {}
...
--有人上庄的时候
function BrniuniuDesk:updateBrnnDelarSeat(m)
    
end

--跟新下注时间
function BrniuniuDesk:updateBetTiem(m)
...
--}
function BrniuniuDesk:updateByBrFllow(m)
...
function BrniuniuDesk:updateBrSitUser(m)
...
function BrniuniuDesk:updateBrSitUserBySomeoneLeave(m)
...
function BrniuniuDesk:updateBrSitDelar(m)
    self:updateBrnnUser(m, true)
end
...
--}
function BrniuniuDesk:updateByBrOver(m)
    -- loga(" -- BrniuniuDesk:updateByBrOver --" .. pb.tostring(m))
    ...
    self:updateBrnnUser(m, false)
end

function BrniuniuDesk:getMyAndDelarWinChips()
...
function BrniuniuDesk:getWinChipsByUin(uin)
    local winChips = 0
...
--判断玩家是赢还是输
function BrniuniuDesk:getPlayerPoolAreaResult(index, targetUin)
...
function BrniuniuDesk:getGameResultForSpecial()
    local tp = 1
    local ts = 1
...
function BrniuniuDesk:updateBrTotalBets(m)
    self.br_my_bets = {}
...
function BrniuniuDesk:updateBrnnUser(m, update_portrait)
    logd("----updateBrnnUser-----")
...
function BrniuniuDesk:updateBrnnChipsPool(m)
...
--百人场判断是否是vip用户
function BrniuniuDesk:judegeBrIsVip(uin)
...
--百人场判断是否处于隐身状态
function BrniuniuDesk:judegeBrIsHiding(uin)
...
function BrniuniuDesk:setRoomID(roomid)
    self.roomid = roomid
end
-------------------jackpot---------------
--判断是否支持jackpot
function BrniuniuDesk:judgeSupportJackpot()
    return 1 == self.jackpot_status
end

--判断是否有人击中百人奖池
function BrniuniuDesk:judgeBrJackpotHited()
...
--判断是否适用第二套chiplist
function BrniuniuDesk:judgeBrUseChipList2()
...
--判定是否已经下注
function BrniuniuDesk:checkXiaZhu()
