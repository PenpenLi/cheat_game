...
function IButton:changeStatus(paras)
    ...
    if paras.coat then
        ...
        local c = cc.Sprite:create(BrRes[paras.coat])
...
function IButton:_genNumberNode( number,fllow , fllowres,point_process)
    ...
    local nl = cc.LabelAtlas:_create(number,BrRes.res020, 27, 42, string.byte('.'))
    ...
    if fllow == true then self:_appendNode(ret,cc.Sprite:create(fllowres or BrRes.res023)) end
    self:_appendNode(ret,nl)
    if m == true then self:_appendNode(ret,cc.Sprite:create(BrRes.res024)) end
    if k == true then self:_appendNode(ret,cc.Sprite:create(BrRes.res021)) end
