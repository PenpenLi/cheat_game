...
function Card:reverseSelf(cb,card, spawn)
    ...
    for i=2,7 do
        ani:addSpriteFrameWithFile(string.format(BrRes.poker_anim, i))
    end
    ...
    -- ani:addSpriteFrameWithFile(filename or BrRes.poker_back)
    ani:setDelayPerUnit(0.04)

    local seq = cc.Sequence:create(cc.Animate:create(ani)
        , cc.CallFunc:create(function( ... )
            -- self:setTexture(filename or BrRes.poker_back)
            if cb then cb() end
