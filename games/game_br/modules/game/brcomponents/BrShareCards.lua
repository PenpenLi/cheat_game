local BrShareCards = class("BrShareCards",function(paras)
    return paras.node
end)
...
BrShareCards.TAG = "BrShareCards"

function BrShareCards:ctor(paras)
    self.winSize = cc.Director:getInstance():getWinSize()
    self:init()
end

function BrShareCards:init()
    for i = 1 , 5 do
...
function BrShareCards:giveCards(delay,cdelay,dpoint)
    delay = delay or 0
    cdelay = cdelay or 0
    self:clearCards()
    self.cards = {}
    ...
    for i = 1 , 5 do
        ...
        self:delayRun(dt[i],function() 
            card.value = Cache.brdesk.br_sharecards[i]
            card:reverseSelf(nil,card.value)
        end)
    end
end

function BrShareCards:delayRun(time,cb)
...
function BrShareCards:clearCards()
