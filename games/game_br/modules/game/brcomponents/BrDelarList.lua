local BrDelarList = class("BrDelarList",CommonWidget.PopupWindow)

BrDelarList.TAG = "BrDelarList"

function BrDelarList:ctor(isExit)
    ...
    self:init()
    self:initTouch()
    qf.event:dispatchEvent(BR_ET.BR_QUERY_BANKER_LIST_CLICK)
...
function BrDelarList:initFullScreenAdaptive()
    local dx = (cc.Director:getInstance():getWinSize().width-1920)/2
    Util:setPosOffset(self.root, {x = dx})
end

function BrDelarList:init()
    self.list_item = self.root:getChildByName("list_item")

    local str = Util:getOldFormatString(Cache.packetInfo:getProMoney(Cache.brdesk.min_banker))
    local min_banker = Util:getProductFormatString(Cache.packetInfo:getProMoney(Cache.brdesk.min_banker))
    local min_pool_banker = Util:getProductFormatString(Cache.packetInfo:getProMoney(Cache.brdesk.min_poor_bank))
    local str = string.format(BrTXT.br_delarlist_txt1,min_banker, min_pool_banker)
...
function BrDelarList:gotoDelar(status)
    self.isExit = status
    if status == true then
...
function BrDelarList:show()
    self.super.show(self)
end

function BrDelarList:initTouch()
    local back = self.root:getChildByName("back_btn")
    local btn = self.root:getChildByName("application_up")
    local backFunc = function ( ... )
        self:close()
    end
    addButtonEvent(back,backFunc)
    Util:enlargeCloseBtnClickArea(back, backFunc)
    ...
    addButtonEvent(btn,function() 
        if self.isExit == true then
            ...
            qf.event:dispatchEvent(BR_ET.BR_DELAR_REQ)
            self.showing = false
        end
    end)
end

function BrDelarList:update()
    if self.root == nil then return end
    self:refreshListView()
    
    --显示前方等待上庄数
    self.root:getChildByName("txt_1"):setString(string.format(BrTXT.br_delarlist_wait_num, self.infoCount))
    self:gotoDelar(Cache.brinfo.be_delaring)  
end


function BrDelarList:refreshListView()
    ...
    local index = 0
    for k,v in pairs(allInfo) do
        if v ~= nil and v.uin == Cache.brdesk.br_delar.uin then 
...
function BrDelarList:updateHead(node,portrait,sex, extparas)
...
function BrDelarList:delayRun(time,cb)
...
function BrDelarList:stopDelayRun()
