local MessageBoxView = class("MessageBoxView", CommonWidget.PopupWindow)

MessageBoxView.TAG = "MessageBoxView"

function MessageBoxView:ctor(parameters)
    self.root = ccs.GUIReader:getInstance():widgetFromJsonFile(GameRes.messageBox)
    ...
    self.super.ctor(self, {id=PopupManager.POPUPWINDOW.messageBox, child=self.root})
...
function MessageBoxView:initDesc( paras )
    ...
    if paras.desc then
        ...
        txtDesc:setColor(cc.c3b(120, 169, 255))
    else
        txtDesc:setString("")
...
function MessageBoxView:initButton( parameters )
    -- body
    Util:addButtonScaleAnimFuncWithDScale(ccui.Helper:seekWidgetByName(self.panelBox,"Button_ok"),function (sender)
    ...
    Util:enlargeCloseBtnClickArea(closeBtn,function (sender)
        if parameters.cbCancel then
            parameters.cbCancel()
        end
        self:close()
