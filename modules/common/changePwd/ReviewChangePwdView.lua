local _ChangePwdView = import(".ChangePwdView")
local ChangePwdView = class("ChangePwdView", _ChangePwdView)
ChangePwdView.TAG = "ChangePwdView"

function ChangePwdView:ctor(parameters)
    self.root = ccs.GUIReader:getInstance():widgetFromJsonFile(GameRes.reviewChangePwdJson)
    self:init(args)
    self.super.super.ctor(self, {id=PopupManager.POPUPWINDOW.changePwd, child=self.root})
end
...
--         local tpos = Util:convertALocalPosToBLocalPos(editFrame, cc.p(0,-editFrame:getContentSize().height/2), listview:getParent())
...
--             -- local labelSend = ccui.Helper:seekWidgetByName(panelInfo,"Label_send")
...
--                     --赠送的钱
--                     -- self:setRichTextContent(Cache.packetInfo:getProMoney(rsp.model.bind_money)  ..GameTxt.string_changepwd_1, self.bindWizard)
--                     -- labelSend:setString(Cache.packetInfo:getProMoney(rsp.model.bind_money)  ..GameTxt.string_changepwd_1)
--                     -- local imageSendFrame = ccui.Helper:seekWidgetByName(panelInfo,"Image_send_frame")
--                  end
--             end
--            --关闭按钮
