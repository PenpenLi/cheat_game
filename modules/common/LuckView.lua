local LuckView = class("LuckView", CommonWidget.PopupWindow)
...
function LuckView:ctor(parameters)
    self.root = ccs.GUIReader:getInstance():widgetFromJsonFile(GameRes.luckJson)
    self:init(parameters)
    self.super.ctor(self, {id=PopupManager.POPUPWINDOW.luckView, child=self.root})
end
...
function LuckView:refreshLuckMainView()
	self.wchat:stopAllActions()
	self.wchat:setVisible(true)
	local hideChat = cc.CallFunc:create(
		function ( ... )
			self.wchat:setVisible(false)
		end
	)
	local showChat = cc.CallFunc:create(
		function ( ... )
			self.wchat:setVisible(true)
		end
	)
	local sequence = cc.Sequence:create(showChat, cc.DelayTime:create(2), hideChat, cc.DelayTime:create(5))
	self.wchat:runAction(cc.RepeatForever:create(sequence))
...
function LuckView:resetInfo()
	...
	self.timeTxt:setText(string.format("%d".. GameTxt.second_unit, 0))
	self.lastInput = ""
end
...
function LuckView:buyMammmon()
	local args = {
		gold = checknumber(self.lastInput),
		uin = Cache.user.uin
	}
	...
	-- dump(args)
	Cache.mammonInfo:requestBuyMammon(args, function (data)
		if data.ltime > 0 then
			Cache.mammonInfo:reCountDown(data.ltime)
			self:showView(ViewTbl.LuckMain)
			qf.event:dispatchEvent(ET.GLOBAL_TOAST,{txt = GameTxt.luck_txt_1})
		end
	end)
end
